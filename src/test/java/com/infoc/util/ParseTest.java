...
public class ParseTest {
    ...
    public void parseSentence() {
        ...
        List<String> sList = Lists.newArrayList(Splitter.onPattern("\\.\\s|다\\.")
                                                        .trimResults()
                                                        .omitEmptyStrings()
                                                        .split(contents)
        );
    ...
    public void testClear() {
        ...
        DateTime dt = formatter.parseDateTime("02/07/2014 12:00:37");

        if (dt.getMonthOfYear() <= currentTime.getMonthOfYear() &&
                dt.getDayOfMonth() < currentTime.getDayOfMonth() &&
                dt.getHourOfDay() <= currentTime.getHourOfDay()) {
            LOG.debug("true");
        } else {
            LOG.debug("false");
        }
